...
COPY appengine.zip /tmp/appengine.zip
# RUN wget --no-verbose -O /tmp/appengine.zip https://storage.googleapis.com/appengine-sdks/featured/appengine-java-sdk-1.9.98.zip
RUN cd opt/appengine && unzip -o /tmp/appengine.zip

# Configure shell
ENV PATH="/opt/appengine/appengine-java-sdk-1.9.98/bin:${PATH}"
...
# Download source
ARG VERSION=master
COPY appinventor-sources/ /opt/src/
